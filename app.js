...
var ACCOUNTS = [
  {username:'admin',password:'admin123',role:'admin',nama:'Administrator Kanwil'},
  {username:'user',password:'user123',role:'user',nama:'Operator Kanwil'},
  {username:'bapas_smr',password:'smr123',role:'bapas',nama:'Operator Bapas Samarinda',unit:'smr',unitLabel:'Bapas Kelas I Samarinda'},
  {username:'bapas_bpp',password:'bpp123',role:'bapas',nama:'Operator Bapas Balikpapan',unit:'bpp',unitLabel:'Bapas Kelas I Balikpapan'},
  {username:'bapas_trk',password:'trk123',role:'bapas',nama:'Operator Bapas Tarakan',unit:'trk',unitLabel:'Bapas Kelas II Tarakan'},
];
...
var DRIVE_LINKS = {
  smr: {
    registrasi_litmas: 'https://drive.google.com/drive/folders/1OqkDAWbm88N0XWYTgDHIGiIR8vr0fxz5?usp=drive_link',
    pengawasan:        'https://drive.google.com/drive/folders/1JEJnE80tIDxFwVE2dua08vR13MplViip?usp=drive_link',
    pendampingan:      'https://drive.google.com/drive/folders/1r-UUG71NKNYp6rkqQqPgfX8sCDc3w2OC?usp=drive_link',
    pembimbingan:      'https://drive.google.com/drive/folders/14KSImoCFf0OTU_fFTlqZRbeyt1uXbVyp?usp=drive_link',
    pelibatan:         'https://drive.google.com/drive/folders/1AvUElQ5ILEJwBRKONmhKzGt5we5GUgln?usp=drive_link',
    infografis_profil: 'https://drive.google.com/drive/folders/1yVJh4as2ksE1EZUFPnLNWEFLZf7Dl74n?usp=drive_link'
  },
  bpp: {
    registrasi_litmas: 'https://drive.google.com/drive/folders/1KIe5vthXH0UGPfucP9d1lM4o6B5fwGG0?usp=drive_link',
    pengawasan:        'https://drive.google.com/drive/folders/13kYQxil-DYeAjFN_uqx31DTu3HmXHdjl?usp=drive_link',
    pendampingan:      'https://drive.google.com/drive/folders/1Ry-dTk2SmXgqEu3oxi-Xd2NQ7iP4ZbFp?usp=drive_link',
    pembimbingan:      'https://drive.google.com/drive/folders/1gYGtP1qso7qgLQQ-1x0OQ2cI69DM6104?usp=drive_link',
    pelibatan:         'https://drive.google.com/drive/folders/1zxN87cFteEXAG9yzibLk5dUBK9xR2OnO?usp=drive_link',
    infografis_profil: 'https://drive.google.com/drive/folders/1XA4dvQqYL7Ne7rKouiUh4mBBksmm1k1_?usp=drive_link'
  },
  trk: {
    registrasi_litmas: 'https://drive.google.com/drive/folders/1KIe5vthXH0UGPfucP9d1lM4o6B5fwGG0?usp=drive_link',
    pengawasan:        'https://drive.google.com/drive/folders/1mszDNHYCWyy6hGvj7psaHqmhuErHnO4m?usp=drive_link',
    pendampingan:      'https://drive.google.com/drive/folders/1VB-RdMdAm8QjqeiX4AOz2IY9sJb8Dlb0?usp=drive_link',
    pembimbingan:      'https://drive.google.com/drive/folders/1QTVF-CC7iRV9sqJkPLsgcOB_wEvjpOWT?usp=drive_link',
    pelibatan:         'https://drive.google.com/drive/folders/1Jws39Z5BGEb40nRiQXLFssFC2zDLJjZW?usp=drive_link',
    infografis_profil: 'https://drive.google.com/drive/folders/1Z4x7fO4K259M-F5EV1He9d45B2eDjNa8?usp=drive_link'
  }
};
...
function doLogin() {
  ...
  try { localStorage.setItem('dashSession', JSON.stringify({username: acc.username, role: acc.role})); } catch(e){}
  document.getElementById('login-screen').style.display='none';
  var app = document.getElementById('app');
  app.style.display='flex'; app.classList.add('ready');
  if (currentRole==='admin') { app.classList.add('is-admin'); app.classList.remove('is-bapas'); }
  else if (currentRole==='bapas') { app.classList.add('is-bapas'); app.classList.remove('is-admin'); }
  else { app.classList.remove('is-admin'); app.classList.remove('is-bapas'); }
  setupUserUI();
  initApp();
...
function doLogout() {
  ...
  try { localStorage.removeItem('dashSession'); } catch(e){}
  document.getElementById('app').style.display='none';
  document.getElementById('app').classList.remove('ready','is-admin','is-bapas');
...
function setupUserUI() {
  ...
  document.getElementById('sb-role-text').textContent = u.role==='admin'?'Administrator':u.role==='bapas'?u.unitLabel||'Operator Bapas':'Operator';
  ...
  av.className = 'su-avatar' + (u.role==='admin'?' admin-av':u.role==='bapas'?' bapas-av':'');
  var badge = document.getElementById('sb-role-badge');
  badge.textContent = u.role==='admin'?'Admin':u.role==='bapas'?'Bapas':'User';
  badge.className = 'su-badge ' + u.role;
  tbav.className = 'tb-avatar' + (u.role==='admin'?' admin':u.role==='bapas'?' bapas':'');
...
function initApp() {
  ...
  // Bapas form + admin badge
  if (currentRole === 'bapas') {
    setTimeout(renderBapasInputForm, 50);
  } else if (currentRole === 'admin') {
    setTimeout(updateAdminBadgeCount, 50);
    setTimeout(renderAdminRekap, 100);
  }
...
function goPage(id, navEl, skipHistory) {
  ...
  if (id==='laporan-harian') {
    if (currentRole==='bapas') setTimeout(renderBapasInputForm, 100);
    else if (currentRole==='admin') { setTimeout(updateAdminBadgeCount, 50); setTimeout(renderAdminRekap, 100); }
  }
...
function updateJftTotal(unit) {
  ...
  var _jftEl = document.getElementById('jft-'+unit+'-total'); if(_jftEl) _jftEl.textContent = total;
}
...
function syncOvJft() {
  ...
  // smr
  var sa=iv('jft_smr_apk'),sp=iv('jft_smr_pertama'),sm=iv('jft_smr_muda'),sma=iv('jft_smr_madya');
  ...
  // bpp
  var ba=iv('jft_bpp_apk'),bp=iv('jft_bpp_pertama'),bm=iv('jft_bpp_muda'),bma=iv('jft_bpp_madya');
  ...
  // trk
  var ta=iv('jft_trk_apk'),tp=iv('jft_trk_pertama'),tm=iv('jft_trk_muda'),tma=iv('jft_trk_madya');
...
function extractBlockTotal(text, keyword) {
  ...
  var re1 = new RegExp(esc+'[\\s\\S]{0,120}?jumlah\\s*[=:]\\s*([\\d.,]+)','i');
...
function parseReport(text){
  ...
  var asimDew = tDew.match(/asimila[si]*[\s\S]{0,120}?jumlah\s*[=:]\s*([\d.,]+)/i);
  d.asim_dewasa = asimDew ? parseInt(asimDew[1].replace(/[.,]/g,''), 10) : null;

  // Klien Bekerja (hanya di bagian dewasa)
  d.bekerja = extractBlockTotal(tDew, 'klien bekerja') ||
              extractNum(tDew, [/klien bekerja[\s\S]{0,120}jumlah\s*[=:]\s*([\d.,]+)/i]);

  // Tipikor (hanya dewasa)
  d.tipikor = extractBlockTotal(tDew, 'klien tipikor') ||
  ...
  var nDewBlock = tDew.match(/klien narkotika([\s\S]{0,150}?)(?=klien bekerja|permintaan|penyelesaian|pib|$)/i);
  var nDewVal = nDewBlock ? extractNum(nDewBlock[0], [/jumlah\s*[=:]\s*([\d.,]+)/i]) : null;
  if (nDewVal === null) {
    var nDew2 = tDew.match(/klien narkotika[\s\S]{0,120}jumlah\s*[=:]\s*([\d.,]+)/i);
    nDewVal = nDew2 ? parseInt(nDew2[1].replace(/[.,]/g,''), 10) : null;
  }

  // Teroris dewasa — potong blok "Klien Teroris" saja
  var terDewBlock = tDew.match(/klien teroris([\s\S]{0,150}?)(?=klien tipikor|klien narkotika|klien bekerja|pib|permintaan|$)/i);
  var terDewVal = terDewBlock ? extractNum(terDewBlock[0], [/jumlah\s*[=:]\s*([\d.,]+)/i]) : null;
  if (terDewVal === null) {
    var terDew2 = tDew.match(/klien teroris[\s\S]{0,120}jumlah\s*[=:]\s*([\d.,]+)/i);
  ...
  var asimAnak = tAnak.match(/asimila[si]*[\s\S]{0,120}?jumlah\s*[=:]\s*([\d.,]+)/i);
  d.asim_anak = asimAnak ? parseInt(asimAnak[1].replace(/[.,]/g,''), 10) : null;
  ...
  var nAnakBlock = tAnak.match(/klien narkotika([\s\S]{0,150}?)(?=teroris|pib|akot|diversi|klien anak sekolah|permintaan|$)/i);
  var nAnakVal = nAnakBlock ? extractNum(nAnakBlock[0], [/jumlah\s*[=:]\s*([\d.,]+)/i]) : null;
  if (nAnakVal === null) {
    var nAnak2 = tAnak.match(/klien narkotika[\s\S]{0,120}jumlah\s*[=:]\s*([\d.,]+)/i);
  ...
  function getSection(fullText, keyword) {
    ...
    for (var i = 0; i < lines.length; i++) {
      if (lines[i].toLowerCase().trim().indexOf(kl) === 0) { start = i; break; }
    }
    if (start < 0) return '';
  ...
  d.narkotika = (nDewVal !== null || nAnakVal !== null) ? (nDewVal||0) + (nAnakVal||0) : null;
  d.teroris   = (terDewVal !== null || terAnakVal !== null) ? (terDewVal||0) + (terAnakVal||0) : null;

  // Litmas anak - format "Permintaan Litmas Anak 2026: 14 orang"
  d.litmas_anak_req  = extractNum(tAnak, [/permintaan litmas anak\s*\d{0,4}\s*[:\-]?\s*([\d.,]+)/i]);
  d.litmas_anak_done = extractNum(tAnak, [/penyelesaian litmas anak\s*\d{0,4}\s*[:\-]?\s*([\d.,]+)/i]);
...
var PREVIEW_FIELDS=[
  // Dewasa
  {key:'dewasa',label:'Klien Dewasa'},{key:'pb_dewasa',label:'PB (Dewasa)'},
  {key:'cb_dewasa',label:'CB (Dewasa)'},{key:'cmb_dewasa',label:'CMB (Dewasa)'},
  {key:'cmk_dewasa',label:'CMK (Dewasa)'},{key:'asim_dewasa',label:'Asimilasi (Dewasa)'},
  {key:'tipikor',label:'Klien Tipikor'},{key:'pib_dewasa',label:'PiB (Dewasa)'},
  ...
  // Anak
  {key:'anak',label:'Klien Anak'},{key:'pb_anak',label:'PB (Anak)'},
  {key:'cb_anak',label:'CB (Anak)'},{key:'cmb_anak',label:'CMB (Anak)'},
  {key:'cmk_anak',label:'CMK (Anak)'},{key:'asim_anak',label:'Asimilasi (Anak)'},
  {key:'latker',label:'LPKS/Latker'},{key:'pib_anak',label:'PiB (Anak)'},
  {key:'akot',label:'AKOT'},{key:'diversi',label:'Diversi'},
  ...
  // Gabungan
  {key:'narkotika',label:'Narkotika (Total)'},{key:'teroris',label:'Teroris (Total)'},
];
...
try { var _saved = localStorage.getItem('totalKlienAktif'); if(_saved) _lastTotalKlien = parseInt(_saved)||0; } catch(e){}
...
function generateLaporan(){
  ...
  if(missing.length>0){errEl.textContent='⚠ Semua laporan Bapas harus diisi. Belum terisi: '+missing.join(', ')+'.';errEl.style.display='block';return;}
  ...
  laporan_count++;
  var _elLH=document.getElementById('ov-laporan-hari'); if(_elLH) _elLH.textContent=laporan_count;
  ...
  if(totalKlien > 0) { _lastTotalKlien = totalKlien; try { localStorage.setItem('totalKlienAktif', totalKlien); } catch(e){} }
  ...
  var existIdx = ARSIP_DB.findIndex(function(a){ return a.tanggal === hari; });
  if (existIdx >= 0) {
    ARSIP_DB[existIdx].dewasa = totalDew;
    ARSIP_DB[existIdx].anak = totalAnak;
    ARSIP_DB[existIdx].laporan = result;
  } else {
    var newId = ARSIP_DB.length ? Math.max.apply(null, ARSIP_DB.map(function(x){return x.id;})) + 1 : 1;
    ARSIP_DB.unshift({id:newId, tanggal:hari, dewasa:totalDew, anak:totalAnak, laporan:result});
  }
...
var FOLDER_KEYS = ['registrasi_litmas','pengawasan','pendampingan','pembimbingan','pelibatan','infografis_profil'];
var FOLDER_META = {
  registrasi_litmas: { label: 'Registrasi &amp; Litmas', ico: '📋', bg: '#f0fdf4' },
  ...
  infografis_profil: { label: 'Data Infografis Profil Bapas', ico: '📁', bg: '#f8fafc' }
};
...
function renderFolderList(unit) {
  ...
  container.innerHTML = FOLDER_KEYS.map(function(k) {
    ...
    var url = links[k] || '';
    if (url) {
      return '<a class="lapbul-folder-row" href="' + url + '" target="_blank" rel="noopener noreferrer"'
        + ' onclick="if(currentUser)logActivity(\'Buka ' + k + ' ' + unit.toUpperCase() + ' oleh \'+currentUser.nama)">'
        + '<div class="lapbul-folder-ico" style="background:' + m.bg + '">' + m.ico + '</div>'
        + '<div class="lapbul-folder-text"><div class="lapbul-folder-name">' + m.label + '</div>'
        + '<div class="lapbul-folder-ext">Excel · Bulanan</div></div>'
        + '<div class="lapbul-folder-arr">↗</div></a>';
    } else {
      return '<div class="lapbul-folder-row lapbul-folder-empty">'
        + '<div class="lapbul-folder-ico" style="background:#f1f5f9;opacity:.5">' + m.ico + '</div>'
        + '<div class="lapbul-folder-text"><div class="lapbul-folder-name" style="opacity:.5">' + m.label + '</div>'
...
function openDriveLinkModal() {
  if (currentRole !== 'admin') { showToast('Hanya admin yang dapat mengubah link Drive.', 'error'); return; }
...
function updateAllDriveBtnState() { renderAllFolderLists(); }
...
function renderArsip(){
  ...
  if(!list) return;
  var q=(document.getElementById('search-arsip')&&document.getElementById('search-arsip').value||'').toLowerCase();
...
function renderOvArsip(){
  ...
  if(list){
    ...
    if(recent.length===0){
      list.innerHTML='<div style="color:var(--text3);font-size:13px;padding:12px 0;">Belum ada laporan harian yang di-generate.</div>';
    } else {
      list.innerHTML=recent.map(function(a){
        return '<div class="arsip-card" onclick="goPage(\'laporan-harian\',null)" style="cursor:pointer;">'
          +'<div class="arsip-month-badge">'+a.tanggal+'</div>'
          +'<h3>Laporan Harian</h3>'
          +'<div class="arsip-meta">'
          +'<span class="arsip-chip">👥 '+fmt(a.dewasa)+' Dewasa</span>'
...
function viewArsip(id){
  ...
  document.getElementById('modal-arsip-title').textContent='Detail Laporan — '+a.tanggal;
  document.getElementById('modal-arsip-body').innerHTML=
    '<div class="detail-section"><h4>Informasi Umum</h4>'
    +'<div class="detail-row"><span>Tanggal</span><span>'+a.tanggal+'</span></div>'
    +'</div>'
...
function copyArsipContent(){
  ...
  var text=(a.laporan||('Laporan Harian '+a.tanggal+'\nKlien Dewasa: '+fmt(a.dewasa)+'\nKlien Anak: '+a.anak+'\nTotal: '+fmt(a.dewasa+a.anak)));
...
function saveProfil() {
  ...
  if (acc) { acc.nama = namaBaru; if (profilFotoData) acc.foto = profilFotoData; }
  var dbu = USERS_DB.find(function(a){ return a.username === currentUser.username; });
  if (dbu) { dbu.nama = namaBaru; if (profilFotoData) dbu.foto = profilFotoData; }
  setupUserUI();
...
(function restoreSession() {
  try {
    ...
    var acc = ACCOUNTS.find(function(a){ return a.username === sess.username && a.role === sess.role; });
    if (!acc) { localStorage.removeItem('dashSession'); return; }
    currentUser = acc;
    currentRole = acc.role;
    document.getElementById('login-screen').style.display = 'none';
    var app = document.getElementById('app');
    ...
    try { localStorage.removeItem('dashSession'); } catch(e2){}
  }
})();
...
/* ===================================================================
   SISTEM LAPORAN HARIAN BAPAS — Input terstruktur per Bapas
   =================================================================== */

/* Storage key format: bapas_laporan_{unit}_{tanggal_iso} */
var BAPAS_DATA_KEY_PREFIX = 'bapas_laporan_';

function getBapasStorageKey(unit, tanggal) {
  var safe = tanggal.replace(/[^a-zA-Z0-9]/g, '_');
  return BAPAS_DATA_KEY_PREFIX + unit + '_' + safe;
}

function saveBapasLaporan(unit, tanggal, data) {
  try {
    localStorage.setItem(getBapasStorageKey(unit, tanggal), JSON.stringify({
      unit: unit,
      tanggal: tanggal,
      data: data,
...
function loadBapasLaporan(unit, tanggal) {
  try {
    var raw = localStorage.getItem(getBapasStorageKey(unit, tanggal));
...
/* Render form input Bapas (untuk role bapas) */
function renderBapasInputForm() {
  ...
  var hari = document.getElementById('meta-hari') ? document.getElementById('meta-hari').value : getTanggalHariIni();
  var existing = loadBapasLaporan(unit, tanggal);
  var d = existing ? existing.data : {};

  var colorMap = {smr:'#2563eb', bpp:'#d97706', trk:'#7c3aed'};
  var color = colorMap[unit] || '#2563eb';

  var fields = [
    {group:'DATA JFT PK', items:[
      {key:'jft_apk',label:'APK'},
      {key:'jft_pertama',label:'PK Pertama'},
      {key:'jft_muda',label:'PK Muda'},
      {key:'jft_madya',label:'PK Madya'},
    ]},
    {group:'KLIEN DEWASA', items:[
      ...
      {key:'bekerja',label:'Klien Bekerja'},
      {key:'narkotika_dewasa',label:'Narkotika (Dewasa)'},
      {key:'teroris_dewasa',label:'Teroris (Dewasa)'},
    ]},
    {group:'LITMAS DEWASA', items:[
      {key:'litmas_dew_req',label:'Permintaan Litmas Dewasa'},
      {key:'litmas_dew_done',label:'Penyelesaian Litmas Dewasa'},
    ]},
    {group:'KLIEN ANAK', items:[
      ...
      {key:'narkotika_anak',label:'Narkotika (Anak)'},
      {key:'teroris_anak',label:'Teroris (Anak)'},
    ]},
    {group:'LITMAS ANAK', items:[
      {key:'litmas_anak_req',label:'Permintaan Litmas Anak'},
      {key:'litmas_anak_done',label:'Penyelesaian Litmas Anak'},
    ]},
  ];
  ...
  if (existing) {
    ...
    statusHtml = '<div class="bapas-status-ok">✅ Data sudah dikirim ke Admin Kanwil pada ' + sat.toLocaleString('id-ID') + '. Anda masih bisa memperbarui data hari ini.</div>';
  } else {
    statusHtml = '<div class="bapas-status-pending">⏳ Belum ada data yang dikirim hari ini. Isi form di bawah lalu tekan <strong>Kirim ke Admin</strong>.</div>';
  }

  var formHtml = '<div class="bapas-form-header" style="border-left:4px solid '+color+'">'
    + '<div class="bapas-form-unit-badge" style="background:'+color+'">'+unitLabel+'</div>'
    + '<div class="bapas-form-date">📅 ' + hari + '</div>'
    + '</div>'
    + statusHtml;

  fields.forEach(function(grp) {
    formHtml += '<div class="bapas-group"><div class="bapas-group-title">'+grp.group+'</div><div class="bapas-field-grid">';
    grp.items.forEach(function(item) {
      var val = d[item.key] !== undefined ? d[item.key] : 0;
      formHtml += '<div class="bapas-field">'
        + '<label>'+item.label+'</label>'
        + '<input type="number" min="0" id="bf_'+item.key+'" value="'+val+'" oninput="bapasFieldChanged()">'
        + '</div>';
    });
    formHtml += '</div></div>';
  });

  formHtml += '<div class="bapas-form-actions">'
...
function bapasFieldChanged() {
  // Auto-save draft ke localStorage sebagai form state
  // (tidak langsung kirim, tunggu tombol submit)
}

function getBapasFormData() {
  var keys = ['jft_apk','jft_pertama','jft_muda','jft_madya',
    'dewasa','pb_dewasa','cb_dewasa','cmb_dewasa','cmk_dewasa','asim_dewasa','pib_dewasa','bekerja','narkotika_dewasa','teroris_dewasa',
    'litmas_dew_req','litmas_dew_done',
    'anak','pb_anak','cb_anak','cmb_anak','cmk_anak','asim_anak','pib_anak','akot','diversi','sekolah','latker','narkotika_anak','teroris_anak',
    'litmas_anak_req','litmas_anak_done'];
  var d = {};
...
function submitBapasLaporan() {
  ...
  if (!u || u.role !== 'bapas') return;
  var tanggal = getTanggalIso();
  var hariTanggal = document.getElementById('meta-hari') ? document.getElementById('meta-hari').value : getTanggalHariIni();
  ...
  if ((data.dewasa||0) + (data.anak||0) === 0) {
    ...
    if (errEl) { errEl.textContent = '⚠ Mohon isi minimal data Klien Dewasa atau Klien Anak.'; errEl.style.display='block'; }
    return;
  }
  var ok = saveBapasLaporan(u.unit, tanggal, data);
  if (ok) {
    showToast('✅ Laporan ' + u.unitLabel + ' berhasil dikirim ke Admin Kanwil!', 'success');
    logActivity('Submit laporan harian oleh ' + u.nama);
    renderBapasInputForm(); // re-render to show status
    // Trigger notif badge di admin
...
function resetBapasForm() {
  ...
  var inputs = document.querySelectorAll('#bapas-form-container input[type=number]');
  inputs.forEach(function(el) { el.value = 0; });
  showToast('Form direset.', 'success');
}

/* ===================================================================
   ADMIN — Rekap dari inputan 3 Bapas
   =================================================================== */

function loadAllBapasData(tanggal) {
  return {
    smr: loadBapasLaporan('smr', tanggal),
    bpp: loadBapasLaporan('bpp', tanggal),
    trk: loadBapasLaporan('trk', tanggal),
  };
}

function updateAdminBadgeCount() {
  var tanggal = getTanggalIso();
  var all = loadAllBapasData(tanggal);
  ...
  if (badge) {
    ...
    badge.className = 'bapas-count-badge ' + (count===3?'full':count>0?'partial':'');
  }
}
...
function renderAdminRekap() {
  ...
  statusHtml = '<div class="admin-bapas-status-row">';
  units.forEach(function(u) {
    var rec = all[u.key];
    if (rec) {
      var sat = new Date(rec.submittedAt);
      statusHtml += '<div class="admin-bapas-card submitted" style="border-left-color:'+u.color+'">'
        + '<div class="abc-title" style="color:'+u.color+'">✅ '+u.label+'</div>'
        + '<div class="abc-time">Dikirim: '+sat.toLocaleTimeString('id-ID', {hour:'2-digit',minute:'2-digit'})+' WITA</div>'
        + '<div class="abc-by">oleh: '+rec.submittedBy+'</div>'
        + '</div>';
    } else {
      statusHtml += '<div class="admin-bapas-card pending" style="border-left-color:#94a3b8">'
        + '<div class="abc-title" style="color:#94a3b8">⏳ '+u.label+'</div>'
  ...
  // Generate rekap otomatis jika ada minimal 1 data
  var anyData = all.smr || all.bpp || all.trk;
  var rekapEl = document.getElementById('admin-rekap-area');
  if (!rekapEl) return;

  if (!anyData) {
    rekapEl.innerHTML = '<div class="rekap-empty">📭 Belum ada data dari Bapas hari ini. Tunggu kiriman data dari masing-masing Bapas.</div>';
    return;
  }

  function gd(rec, key) { return (rec && rec.data && rec.data[key]) ? (parseInt(rec.data[key])||0) : 0; }

  // Build parsed data objects untuk buildLaporan
  function makeParseData(rec) {
    if (!rec) return {};
    var d = rec.data || {};
    return {
      dewasa: d.dewasa||0, anak: d.anak||0,
      pb_dewasa: d.pb_dewasa||0, cb_dewasa: d.cb_dewasa||0,
      cmb_dewasa: d.cmb_dewasa||0, cmk_dewasa: d.cmk_dewasa||0,
      asim_dewasa: d.asim_dewasa||0, pib_dewasa: d.pib_dewasa||0,
      bekerja: d.bekerja||0, narkotika: (d.narkotika_dewasa||0)+(d.narkotika_anak||0),
      teroris: (d.teroris_dewasa||0)+(d.teroris_anak||0),
      pb_anak: d.pb_anak||0, cb_anak: d.cb_anak||0,
      cmb_anak: d.cmb_anak||0, cmk_anak: d.cmk_anak||0,
      asim_anak: d.asim_anak||0, pib_anak: d.pib_anak||0,
      akot: d.akot||0, diversi: d.diversi||0, sekolah: d.sekolah||0, latker: d.latker||0,
      litmas_dew_req: d.litmas_dew_req||0, litmas_dew_done: d.litmas_dew_done||0,
      litmas_anak_req: d.litmas_anak_req||0, litmas_anak_done: d.litmas_anak_done||0,
    };
  }

  var smrData = makeParseData(all.smr);
  var bppData = makeParseData(all.bpp);
  var trkData = makeParseData(all.trk);

  // JFT dari data Bapas + form Kanwil
  var jft = buildJft();
  // Override JFT Bapas dari data yang dikirim
  if (all.smr && all.smr.data) {
    var sd = all.smr.data;
    var smrJft = (sd.jft_apk||0)+(sd.jft_pertama||0)+(sd.jft_muda||0)+(sd.jft_madya||0);
    if (smrJft > 0) {
      jft.smr_apk = sd.jft_apk||0; jft.smr_pertama = sd.jft_pertama||0;
      jft.smr_muda = sd.jft_muda||0; jft.smr_madya = sd.jft_madya||0;
      jft.smr_total = smrJft;
    }
  }
  if (all.bpp && all.bpp.data) {
    var bd = all.bpp.data;
    var bppJft = (bd.jft_apk||0)+(bd.jft_pertama||0)+(bd.jft_muda||0)+(bd.jft_madya||0);
    if (bppJft > 0) {
      jft.bpp_apk = bd.jft_apk||0; jft.bpp_pertama = bd.jft_pertama||0;
      jft.bpp_muda = bd.jft_muda||0; jft.bpp_madya = bd.jft_madya||0;
      jft.bpp_total = bppJft;
    }
  }
  if (all.trk && all.trk.data) {
    var td = all.trk.data;
    var trkJft = (td.jft_apk||0)+(td.jft_pertama||0)+(td.jft_muda||0)+(td.jft_madya||0);
    if (trkJft > 0) {
      jft.trk_apk = td.jft_apk||0; jft.trk_pertama = td.jft_pertama||0;
      jft.trk_muda = td.jft_muda||0; jft.trk_madya = td.jft_madya||0;
  ...
  var hari = document.getElementById('meta-hari') ? document.getElementById('meta-hari').value : getTanggalHariIni();
  var pukul = document.getElementById('meta-pukul') ? document.getElementById('meta-pukul').value : '08.00 WITA';
  var nama = document.getElementById('meta-nama') ? document.getElementById('meta-nama').value : 'Huzaifah Makmur Hidayah';
  var nip = document.getElementById('meta-nip') ? document.getElementById('meta-nip').value : '197505241999021001';

  var result = buildLaporan(smrData, bppData, trkData, jft, hari, pukul, nama, nip);
  lastResult = result;
  ...
  var warningHtml = missingUnits.length > 0
    ? '<div class="rekap-warning">⚠ Data belum masuk dari: <strong>'+missingUnits.join(', ')+'</strong>. Rekap memakai nilai 0 untuk Bapas tersebut.</div>'
    : '<div class="rekap-complete">✅ Data dari semua 3 Bapas sudah masuk. Rekap lengkap!</div>';

  rekapEl.innerHTML = warningHtml
    ...
    + '</div></div>'
    + '<div class="result-body"><pre id="result-text">'+escHtml(result)+'</pre></div>'
    + '</div>';

  // Update arsip
  var totalDew = (smrData.dewasa||0)+(bppData.dewasa||0)+(trkData.dewasa||0);
  var totalAnak = (smrData.anak||0)+(bppData.anak||0)+(trkData.anak||0);
  var existIdx = ARSIP_DB.findIndex(function(a){ return a.tanggal === hari; });
  if (existIdx >= 0) {
    ARSIP_DB[existIdx].dewasa = totalDew;
    ARSIP_DB[existIdx].anak = totalAnak;
    ...
    var newId2 = ARSIP_DB.length ? Math.max.apply(null, ARSIP_DB.map(function(x){return x.id;})) + 1 : 1;
    ARSIP_DB.unshift({id:newId2, tanggal:hari, dewasa:totalDew, anak:totalAnak, laporan:result});
  }
  renderOvArsip();
  laporan_count++;
  var elLH = document.getElementById('ov-laporan-hari'); if(elLH) elLH.textContent = laporan_count;
  showToast('Rekap laporan berhasil di-generate! ✓','success');
  logActivity('Generate rekap laporan harian oleh '+currentUser.nama);
}
...
function clearBapasDataToday() {
  ...
  if (!confirm('Reset semua data kiriman Bapas hari ini?')) return;
  var tanggal = getTanggalIso();
  ['smr','bpp','trk'].forEach(function(unit) {
    try { localStorage.removeItem(getBapasStorageKey(unit, tanggal)); } catch(e){}
  });
  renderAdminRekap();
  updateAdminBadgeCount();
  showToast('Data hari ini direset.','success');
