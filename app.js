...
function initApp() {
  ...
  setTimeout(function () {
    ...
    if (currentRole === "bapas") {
      ...
      setTimeout(function () {
        startAdminRealtimeListener(); // real-time: data masuk otomatis
...
function buildLaporan(smr, bpp, trk, jft, hari, pukul, nama, nip) {
  ...
  out +=
    ...
    " orang\n•Dewasa\t\t:  " +
    fmt(smrDew) +
    " orang (L: " + n(smrData.dewasa_l) + " / P: " + n(smrData.dewasa_p) + ")" +
    "\n•Anak\t\t:  " +
    smrAnak +
    " orang (L: " + n(smrData.anak_l) + " / P: " + n(smrData.anak_p) + ")" +
    "\n\n";
  out +=
    zwj +
    ...
    " orang\n•Dewasa\t\t:  " +
    fmt(bppDew) +
    " orang (L: " + n(bppData.dewasa_l) + " / P: " + n(bppData.dewasa_p) + ")" +
    "\n•Anak\t\t:  " +
    bppAnak +
    " orang (L: " + n(bppData.anak_l) + " / P: " + n(bppData.anak_p) + ")" +
    "\n\n";
  out +=
    zwj +
    ...
    " orang\n•Dewasa\t\t: " +
    fmt(trkDew) +
    " orang (L: " + n(trkData.dewasa_l) + " / P: " + n(trkData.dewasa_p) + ")" +
    "\n•Anak\t\t: " +
    trkAnak +
    " orang (L: " + n(trkData.anak_l) + " / P: " + n(trkData.anak_p) + ")" +
    "\n";
  ...
  units.forEach(function (u) {
    out +=
      u.label +
      "\n•Asimilasi \t: " +
      n(u.data.asim_dewasa) + " orang (L: " + n(u.data.asim_dewasa_l) + " / P: " + n(u.data.asim_dewasa_p) + ")" +
      "\n•PB            \t: " +
      fmt(n(u.data.pb_dewasa)) + " orang (L: " + n(u.data.pb_dewasa_l) + " / P: " + n(u.data.pb_dewasa_p) + ")" +
      "\n•CB            \t: " +
      n(u.data.cb_dewasa) + " orang (L: " + n(u.data.cb_dewasa_l) + " / P: " + n(u.data.cb_dewasa_p) + ")" +
      "\n•CMB         \t: " +
      n(u.data.cmb_dewasa) + " orang (L: " + n(u.data.cmb_dewasa_l) + " / P: " + n(u.data.cmb_dewasa_p) + ")" +
      "\n•CMK         \t: " +
      n(u.data.cmk_dewasa) + " orang (L: " + n(u.data.cmk_dewasa_l) + " / P: " + n(u.data.cmk_dewasa_p) + ")" +
      "\n•PiB           \t: " +
      n(u.data.pib_dewasa) + " orang (L: " + n(u.data.pib_dewasa_l) + " / P: " + n(u.data.pib_dewasa_p) + ")" +
      "\n\n";
  });
  ...
  units.forEach(function (u) {
    out +=
      u.label +
      "\n•Asimilasi \t: " +
      n(u.data.asim_anak) + " orang (L: " + n(u.data.asim_anak_l) + " / P: " + n(u.data.asim_anak_p) + ")" +
      "\n•PB            \t: " +
      n(u.data.pb_anak) + " orang (L: " + n(u.data.pb_anak_l) + " / P: " + n(u.data.pb_anak_p) + ")" +
      "\n•CB            \t: " +
      n(u.data.cb_anak) + " orang (L: " + n(u.data.cb_anak_l) + " / P: " + n(u.data.cb_anak_p) + ")" +
      "\n•CMB         \t: " +
      n(u.data.cmb_anak) + " orang (L: " + n(u.data.cmb_anak_l) + " / P: " + n(u.data.cmb_anak_p) + ")" +
      "\n•CMK         \t: " +
      n(u.data.cmk_anak) + " orang (L: " + n(u.data.cmk_anak_l) + " / P: " + n(u.data.cmk_anak_p) + ")" +
      "\n•PiB           \t: " +
      n(u.data.pib_anak) + " orang (L: " + n(u.data.pib_anak_l) + " / P: " + n(u.data.pib_anak_p) + ")" +
      "\n\n";
  });
...
function initFirebase() {
  try {
    if (typeof firebase === "undefined") {
      ...
      showFirebaseError(
        "Firebase SDK tidak ditemukan. Periksa koneksi internet dan konfigurasi.",
      );
      return;
    }
    if (!_fbApp) {
...
function showFirebaseError(msg) {
  // Update semua elemen firebase-status yang ada
  ["firebase-status", "firebase-status-admin"].forEach(function(id) {
    var el = document.getElementById(id);
    if (el) {
      el.textContent = "⚠ " + msg;
...
function startAdminRealtimeListener() {
  if (!_fbReady || !_fbDb) {
    // Retry setelah 500ms jika Firebase belum siap
    setTimeout(startAdminRealtimeListener, 500);
    return;
  }
  stopAdminRealtimeListener();
  ...
  _fbListenerUnsubscribe = ref.on(
    ...
    function (err) {
      ...
      if (statusEl) statusEl.innerHTML = '<div style="color:#dc2626;font-size:13px;">⚠ Gagal memuat data: ' + err.message + '</div>';
    },
  );
}
...
function renderBapasInputForm() {
  ...
  // Grup dengan L/P: type "lp" = input laki+perempuan+total otomatis
  // Grup biasa: type "single" = satu input angka
  var fields = [
    {
      group: "DATA JFT PK",
      type: "single",
  ...
  loadBapasLaporan(unit, tanggal, function (existing) {
    ...
    fields.forEach(function (grp) {
      ...
      if (grp.type === "lp") {
        // Header tabel L/P
        formHtml += '<div class="bapas-lp-table">' +
          '<div class="bapas-lp-header">' +
          '<div class="bapas-lp-col-label">Kategori</div>' +
          '<div class="bapas-lp-col">♂ Laki-laki</div>' +
          '<div class="bapas-lp-col">♀ Perempuan</div>' +
          '<div class="bapas-lp-col bapas-lp-col-total">Total</div>' +
          '</div>';
        grp.items.forEach(function (item) {
          var valL = d[item.key + "_l"] !== undefined ? d[item.key + "_l"] : 0;
          var valP = d[item.key + "_p"] !== undefined ? d[item.key + "_p"] : 0;
          var valTotal = d[item.key] !== undefined ? d[item.key] : (valL + valP);
          formHtml +=
            '<div class="bapas-lp-row">' +
            '<div class="bapas-lp-row-label">' + item.label + '</div>' +
            '<div class="bapas-lp-col">' +
            '<input type="number" min="0" id="bf_' + item.key + '_l" value="' + valL + '" ' +
            'oninput="bfCalcTotal(\'' + item.key + '\')" class="bf-laki">' +
            '</div>' +
            '<div class="bapas-lp-col">' +
            '<input type="number" min="0" id="bf_' + item.key + '_p" value="' + valP + '" ' +
            'oninput="bfCalcTotal(\'' + item.key + '\')" class="bf-perempuan">' +
            '</div>' +
            '<div class="bapas-lp-col bapas-lp-col-total">' +
            '<div class="bapas-lp-total" id="bf_total_' + item.key + '">' + valTotal + '</div>' +
            '</div>' +
            '</div>';
        });
        formHtml += '</div>';
      } else {
        formHtml += '<div class="bapas-field-grid">';
        grp.items.forEach(function (item) {
          var val = d[item.key] !== undefined ? d[item.key] : 0;
          formHtml +=
            '<div class="bapas-field">' +
            "<label>" + item.label + "</label>" +
            '<input type="number" min="0" id="bf_' + item.key + '" value="' + val + '">' +
            "</div>";
        });
        formHtml += '</div>';
      }
...
// Hitung total L+P dan tampilkan di kolom Total
function bfCalcTotal(key) {
  var elL = document.getElementById("bf_" + key + "_l");
  var elP = document.getElementById("bf_" + key + "_p");
  var elT = document.getElementById("bf_total_" + key);
  if (!elL || !elP || !elT) return;
  var total = (parseInt(elL.value) || 0) + (parseInt(elP.value) || 0);
  elT.textContent = total;
}

function getBapasFormData() {
  // Field JFT dan Litmas: single input
  var singleKeys = [
    "jft_apk", "jft_pertama", "jft_muda", "jft_madya",
    "litmas_dew_req", "litmas_dew_done",
    "litmas_anak_req", "litmas_anak_done",
  ];
  // Field klien dewasa & anak: L+P+Total
  var lpKeys = [
    "dewasa", "pb_dewasa", "cb_dewasa", "cmb_dewasa", "cmk_dewasa",
    "asim_dewasa", "pib_dewasa", "bekerja", "narkotika_dewasa", "teroris_dewasa",
    "anak", "pb_anak", "cb_anak", "cmb_anak", "cmk_anak",
    "asim_anak", "pib_anak", "akot", "diversi", "sekolah", "latker",
    "narkotika_anak", "teroris_anak",
  ];
  var d = {};
  singleKeys.forEach(function (k) {
    var el = document.getElementById("bf_" + k);
    d[k] = el ? parseInt(el.value) || 0 : 0;
  });
  lpKeys.forEach(function (k) {
    var elL = document.getElementById("bf_" + k + "_l");
    var elP = document.getElementById("bf_" + k + "_p");
    var l = elL ? parseInt(elL.value) || 0 : 0;
    var p = elP ? parseInt(elP.value) || 0 : 0;
    d[k + "_l"] = l;
    d[k + "_p"] = p;
    d[k] = l + p; // total
  });
...
function submitBapasLaporan() {
  ...
  if ((data.dewasa || 0) + (data.anak || 0) === 0) {
    ...
    if (errEl) {
      errEl.textContent =
        "⚠ Mohon isi minimal data Klien Dewasa (L atau P) atau Klien Anak (L atau P).";
      errEl.style.display = "block";
    }
    return;
  }
...
function resetBapasForm() {
  ...
  // Reset semua tampilan total L+P
  var totals = document.querySelectorAll("#bapas-form-container .bapas-lp-total");
  totals.forEach(function (el) { el.textContent = "0"; });
...
function _renderAdminRekapFromData(all) {
  ...
  function makeParseData(rec) {
    ...
    var d = rec.data || {};
    function lp(key) {
      // Jika ada data _l/_p gunakan, jika tidak pecah total jadi 0 L dan total P
      return {
        l: d[key + "_l"] !== undefined ? (d[key + "_l"] || 0) : 0,
        p: d[key + "_p"] !== undefined ? (d[key + "_p"] || 0) : 0,
        t: d[key] || 0,
      };
    }
    return {
      dewasa: d.dewasa || 0,
      dewasa_l: (d.dewasa_l || 0),
      dewasa_p: (d.dewasa_p || 0),
      anak: d.anak || 0,
      anak_l: (d.anak_l || 0),
      anak_p: (d.anak_p || 0),
      pb_dewasa: d.pb_dewasa || 0, pb_dewasa_l: d.pb_dewasa_l||0, pb_dewasa_p: d.pb_dewasa_p||0,
      cb_dewasa: d.cb_dewasa || 0, cb_dewasa_l: d.cb_dewasa_l||0, cb_dewasa_p: d.cb_dewasa_p||0,
      cmb_dewasa: d.cmb_dewasa || 0, cmb_dewasa_l: d.cmb_dewasa_l||0, cmb_dewasa_p: d.cmb_dewasa_p||0,
      cmk_dewasa: d.cmk_dewasa || 0, cmk_dewasa_l: d.cmk_dewasa_l||0, cmk_dewasa_p: d.cmk_dewasa_p||0,
      asim_dewasa: d.asim_dewasa || 0, asim_dewasa_l: d.asim_dewasa_l||0, asim_dewasa_p: d.asim_dewasa_p||0,
      pib_dewasa: d.pib_dewasa || 0, pib_dewasa_l: d.pib_dewasa_l||0, pib_dewasa_p: d.pib_dewasa_p||0,
      bekerja: d.bekerja || 0, bekerja_l: d.bekerja_l||0, bekerja_p: d.bekerja_p||0,
      narkotika: (d.narkotika_dewasa || 0) + (d.narkotika_anak || 0),
      teroris: (d.teroris_dewasa || 0) + (d.teroris_anak || 0),
      pb_anak: d.pb_anak || 0, pb_anak_l: d.pb_anak_l||0, pb_anak_p: d.pb_anak_p||0,
      cb_anak: d.cb_anak || 0, cb_anak_l: d.cb_anak_l||0, cb_anak_p: d.cb_anak_p||0,
      cmb_anak: d.cmb_anak || 0, cmb_anak_l: d.cmb_anak_l||0, cmb_anak_p: d.cmb_anak_p||0,
      cmk_anak: d.cmk_anak || 0, cmk_anak_l: d.cmk_anak_l||0, cmk_anak_p: d.cmk_anak_p||0,
      asim_anak: d.asim_anak || 0, asim_anak_l: d.asim_anak_l||0, asim_anak_p: d.asim_anak_p||0,
      pib_anak: d.pib_anak || 0, pib_anak_l: d.pib_anak_l||0, pib_anak_p: d.pib_anak_p||0,
      akot: d.akot || 0, akot_l: d.akot_l||0, akot_p: d.akot_p||0,
      diversi: d.diversi || 0, diversi_l: d.diversi_l||0, diversi_p: d.diversi_p||0,
      sekolah: d.sekolah || 0, sekolah_l: d.sekolah_l||0, sekolah_p: d.sekolah_p||0,
      latker: d.latker || 0, latker_l: d.latker_l||0, latker_p: d.latker_p||0,
      litmas_dew_req: d.litmas_dew_req || 0,
      litmas_dew_done: d.litmas_dew_done || 0,
      litmas_anak_req: d.litmas_anak_req || 0,
      litmas_anak_done: d.litmas_anak_done || 0,
    };
  }
